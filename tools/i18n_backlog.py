...
方針（ユーザー厳命・2026-08-14 更新）:
  - 【新】新規UI文字列は ja.json だけでなく en.json・ne.json にも同時に入れる（英語+ネパール語は必須）。
    en/ne は母語として常時選択でき表示言語になるため、ja だけだと日本語が出るバグになる。
    番人＝ src/i18n/parity.test.ts（ja の全キーが en/ne に有るか＋プレースホルダ一致を検査）。
  - 他8言語(bn/id/ko/my/th/vi/zh)は従来どおりバックログ運用＝「ユーザーの明示指示がある時だけ」まとめて翻訳。
    仕様変更が頻繁なため、都度・先回りの翻訳は禁止（無駄になる）。t() が ja へ自動フォールバック。
