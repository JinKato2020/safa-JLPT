...
## 一意性の自己申告（uniqRisk / uniqNote）※感情語・近接類義が密集する語で必須
一意性の最終判断はユーザーがExcelで行う。あなたは**各問に自己申告を付ける**：
- 正解が本当に1つに定まると自信があれば **uniqRisk は付けない**（欄を出さない）。
- 誤答のどれかが「対象語でもギリギリ成立しうる／近接類義が同義すぎて危うい」と少しでも感じたら **uniqRisk="high"**（危険）または **"mid"**（要確認）を付け、**uniqNote に理由を1文**（どの誤答がなぜ危ういか）。
- 特に**感情・状態を表す語**は類義が密集し「第2の正解」事故（★実バグ=探す）が起きやすい。自信が持てない場合は無理に一意化せず high を付けて申告する（ユーザーが問題ごと落とす判断をする）。
...
4. 個人名・ルビ・括弧補足が無いか。
5. 一意性に少しでも不安があれば uniqRisk/uniqNote を付けたか。'''
...
GEN_SCHEMA = {
    ...
    'properties': {'items': {'type': 'array', 'items': {
        ...
        'properties': {
            ...
            'uniqRisk': {'type': 'string', 'enum': ['high', 'mid'], 'description': '一意性に不安がある時だけ付ける(自信があれば省略)'},
            'uniqNote': {'type': 'string', 'description': 'uniqRiskを付けた理由1文(どの誤答がなぜ危ういか)'},
        }}}},
}
