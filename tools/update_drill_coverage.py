...
NOTE = {
    ...
    '意味を選ぶ(漢字)': '字→意味を4択(認識テスト)。意味が立つ字(訓読みあり or 単独語=meaningClear)だけが対象。校のような字は意味面を作れず対象外。',
    '読み(漢字)': '字→読みを4択(認識テスト)。全字が対象。訓読みを優先し送り仮名を補って提示(強→つよ（い）)。',
    '形の弁別(漢字)': '似た字4択→正しい字を選ぶ(形の弁別)。見た目が紛らわしい字が3つ揃う字(formMakeable)だけが対象。※書き取り(手書き産出)は練習ゆえ面に非計上。',
}
