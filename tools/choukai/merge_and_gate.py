...
def main():
    ...
    for cat in ['kadai','point','gaiyou','hatsuwa','sokuji']:
        ...
        for r in recs:
            ...
            # 役割はラベルに入れず導入文で述べる規約(2026-08-14 実データ照合で確定=既存520問は全てこの方式)。
            # ラベル=「女（おんな）1：/男（おとこ）1：」のみ。数字の直後に（…）が付く=女1（店員）＝役割ラベル→導入文へ移す(表示一貫性)。
            # ふりがな（女(おんな)1）は数字の"前"なので誤検出しない。
            role_label = next((ln.strip() for ln in str(sc).split('\n')
                               if re.match(r'^[男女](?:（[^）]*）)?[0-9]+（[^）]*）\s*[：:]', ln.strip())), None)
            if role_label:
                problems.append(f'{tag}: 役割ラベル「{role_label[:16]}」→ラベルは女1：/男1：のみ・役割は導入文で述べる(2026-08-14規約・参考)')
