...
3. 実装（プラン承認済・復習=10問）：
   - ✅ **P1**（純ロジック）=`src/review/facetMap.ts`＋`facetMastery.ts`＋各test（16件）。全数346緑・tsc0。コミット予定。
   - ⬜ P2状態合流→P3移行→P4復習エンジン→P5予想得点配線→P6 UI入口+旧mixed撤去→P7書き取り2ターン→P8検証

## 次の一手
= **P2**（`src/store/state.ts`に`mastery?`/`masteryMigrated?`追加→`store.tsx`のQUIZ_ANSWER/MOCK_ANSWER/KAKITORI_PROGRESS等の各caseで`recordFacet(facetsForUnit(unit),...)`を合流）。additive＝従来`state.items`記録は残す。
