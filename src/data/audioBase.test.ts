...
test('AUDIO_BASE_URL は既存の聴解と同じ配信ルート', () => {
  assert.equal(AUDIO_BASE_URL, 'https://jlpt.safa-lang.com/assets/audio/');
});

test('vocabAudioUrl は vocab/ サブディレクトリの mp3 を指す', () => {
  assert.equal(vocabAudioUrl('n5-v-1'), 'https://jlpt.safa-lang.com/assets/audio/vocab/n5-v-1.mp3');
  assert.equal(vocabAudioUrl('n3-v-999'), 'https://jlpt.safa-lang.com/assets/audio/vocab/n3-v-999.mp3');
});
