...
test('bankLevelOf: 実データのN4エントリ(kb-003475)の級を正しく逆引きする', () => {
  const entry = (KB as { id: string; level: string }[]).find((b) => b.id === 'kb-003475');
  assert.ok(entry, 'kb-003475 が grammar_form_N4.json に存在すること');
  assert.equal(entry!.level, 'N4');
  assert.equal(bankLevelOf('kb-003475'), 'N4');
});
