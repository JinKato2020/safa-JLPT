...
// ★読み(ルビ)の中身は「ひらがな(＋長音符)のみ」に限定。情報検索の表など括弧を“注記”に使う文
//   (例 午前（10:00〜12:00）/ 甘い（辛くない）/（土曜日（どようび）))で、以前の [^）)]* が数字や漢字入りの
//   括弧まで読みと誤認し「ルビ・表がおかしい」不具合になっていた。ひらがな限定で注記はそのまま本文描画になる。
// ※カタカナ後の（…）でも中身がかな以外(例: カレー（辛口）の説明)なら第2群に一致せず、注記としてそのまま残す。
const RUBY_RE = /([一-鿿㐀-䶿々〆〇ヶ]+)[（(]([ぁ-ゖー]+)[）)]|([ァ-ヴーヵヶ・]+)[（(][ぁ-んァ-ヴー]+[）)]|([\s\S])/gu;
