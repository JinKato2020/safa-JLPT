...
export interface Settings {
  ...
  shareWords?: boolean;    // 自分のmy単語帳を町の相手に見せてよいか。未設定/true=見せる(既定ON)。false=会話画面で相手から単語帳ボタンが出ない
  townShowFriends?: boolean; // 日本語学習者の町に友だちを登場させるか(旧・一括トグル)。未設定/true=表示(既定ON)。※現行UIは友だち個別トグル(townHiddenFriends)へ移行。
  townHiddenFriends?: string[]; // 町に登場させない友だちの user_id 一覧(個別選択)。未設定/空=全員表示(既定)。含まれる相手だけ町に出さない。
