...
// gWeightMul: 文法(grammar)認識面のみ更新強度に掛ける倍率(既定1)。未着手の出題形式ほど大きく=ソフトnovelty重み。
function applyTarget(prev: ItemState | undefined, t: FacetTarget, correct: boolean, signal: 'practice' | 'mock', now: number, gWeightMul = 1): ItemState | null {
  const cur = prev ?? newItemState(now);
  if (t.weight >= 1) {
    // 認識面: 大問由来。失敗で減点＋SRSリセット(engineの標準挙動)。文法だけ形式novelty倍率を掛ける。
    const mul = t.facet === 'grammar' ? gWeightMul : 1;
    return signal === 'mock' ? recordMock(cur, correct, now, mul) : recordQuiz(cur, correct, now, false, mul);
  }
 ...
 */
export function recordFacet(m: MasterySlice, targets: FacetTarget[], correct: boolean, signal: 'practice' | 'mock', now: number, gWeightMul = 1): MasterySlice {
  if (!targets.length) return m;
  let next = m;
  for (const t of targets) {
    const updated = applyTarget(next[t.itemId]?.[t.facet], t, correct, signal, now, gWeightMul);
