...
- **対象 N3/N4のみ。N5＝公式未学習で除外・保留。文章の文法（passage_grammar）＝後回し。用法（usage）＝完了済み（触らない）。**
...
**Task 0（共通エンジン）→ A（組み立て・軽）→ C（言い換え・中）→ B（文脈規定・大）→ D（文法形式・最難）**

理由：軽い・安いものから確実に緑にし、動的化の下地（Task 0）を早期に通す。B/D は各々単独でクォータ見積もり承認。文法持ち込み（保留中）はD完了（pointId整理）後に解禁。
