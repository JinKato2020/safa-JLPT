...
- [x] KakitoriScreen 2ターン化 = **実装済み(ユーザー指定仕様2026-08-01)**。①学習ターン=5字を1字ずつ「なぞり/見て書く」スイッチで選んで練習→[次へ]、②テストターン=同じ5字を見ずに書いて採点→合格でwrite面(step3)へ。学習ターンは記録せず・テストのみrecordKakitori(step3)。自由練習(単字)は従来の3モード直接選択を維持。turn/learnMode状態で制御・全数372緑/tsc0。**未ビルド**。
