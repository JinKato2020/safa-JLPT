...
begin
  ...
  insert into public.friend_profiles(user_id, nickname, country, gender, avatar, level,
      streak, learned, week_learned, study_seconds, studying, strong, personality, mood_msg, updated_at)
...
grant execute on function public.friend_publish(text,text,text,text,text,int,int,int,bigint,text,text,text,text) to authenticated;
